...
PointLight::PointLight(uint16_t& lightNum, std::vector<PointLight*>& container, uint32_t& objectNum)
	: Light(),
...
PointLight::PointLight(uint16_t& lightNum, std::vector<PointLight*>& container, uint32_t& objectNum, glm::vec3 position, float constant, float linear, float quadratic, glm::vec3 ambient, glm::vec3 diffuse, glm::vec3 specular)
...
void PointLight::Draw(Shader* lightShader, Shader* iconShader) {
